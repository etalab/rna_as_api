...
describe RNAAsAPIInteractor do
  ...
  context 'when outputing success logs' do
    it 'output the right format' do
      expect { described_class.new.stdout_success_log('tEsT MesSage') }.to output("       \e[0;32;49m✓  Test message\e[0m\n").to_stdout
    end
  ...
  context 'when outputing warn logs' do
    it 'output the right format' do
      expect { described_class.new.stdout_warn_log('tEsT MesSage') }.to output("       \e[0;33;49m⚠ Test message\e[0m\n").to_stdout
    end
  ...
  context 'when outputing error logs' do
    it 'output the right format' do
      expect { described_class.new.stdout_error_log('tEsT MesSage') }.to output("       \e[0;31;49m✖ Test message\e[0m\n").to_stdout
    end
